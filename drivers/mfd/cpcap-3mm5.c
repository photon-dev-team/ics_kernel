...
#ifdef CONFIG_MFD_CPCAP_AJ_CHARGER_WORKAROUND
static unsigned int headset_status;
static void headset_workaround_work(struct work_struct *);
/* mutex for workaround to prevent race condition */
struct mutex aj_workaround_lock;
#endif
...
#ifdef CONFIG_MFD_CPCAP_AJ_CHARGER_WORKAROUND
	struct work_struct work_aj_workaround;
#endif
};
...
#ifdef CONFIG_MFD_CPCAP_AJ_CHARGER_WORKAROUND
		headset_status = 0;
		schedule_work(&data_3mm5->work_aj_workaround);
#endif
	} else {
...
static void headset_workaround_work(struct work_struct *work)
{
	struct cpcap_3mm5_data *data_3mm5 =
		container_of(work, struct cpcap_3mm5_data, work_aj_workaround);

	mutex_lock(&aj_workaround_lock);
	cpcap_charger_aj_workaround(data_3mm5->cpcap,
				CPCAP_AJ_NOTIFICATION, headset_status);
	mutex_unlock(&aj_workaround_lock);
}
...
#ifdef CONFIG_MFD_CPCAP_AJ_CHARGER_WORKAROUND
	mutex_init(&aj_workaround_lock);
	INIT_WORK(&data->work_aj_workaround, headset_workaround_work);
#endif

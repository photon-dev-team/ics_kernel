...
struct isl29030_data {
	...
	/* Used to make sure no sequential enables/disables */
	int	irqCount;
	struct mutex lock_irq;    /* Mutex to lock access to IRQ */
...
static void isl29030_disable_irq_nosync(struct isl29030_data *isl, int irq)
{
	if (isl->irqCount > 0) {
		mutex_lock(&isl->lock_irq);

		disable_irq_nosync(irq);
		isl->irqCount--;

		mutex_unlock(&isl->lock_irq);
	}
	return;
}

static void isl29030_enable_irq(struct isl29030_data *isl, int irq)
{
	if (isl->irqCount == 0) {
		mutex_lock(&isl->lock_irq);
...
static void isl29030_enable_irq_wake(struct isl29030_data *isl, int irq)
{
	if (isl->irqCount == 0) {
		mutex_lock(&isl->lock_irq);
...
irqreturn_t ld_isl29030_irq_handler(int irq, void *dev)
{
	...
	isl29030_disable_irq_nosync(isl, isl->client->irq);
	queue_work(isl->working_queue, &isl->wq);
	isl29030_enable_irq(isl, isl->client->irq);
...
int isl29030_disable(struct isl29030_data *isl)
{
	...
	if (atomic_cmpxchg(&isl->prox_enabled, 1, 0)) {
		...
		if (atomic_read(&isl->als_enabled) == 0) {
			if (isl29030_debug & ISL29030_DBG_ENABLE_DISABLE)
				pr_info("%s: ALS is not enabled", __func__);
			     ...
			     const struct i2c_device_id *id)
{
	...
	mutex_init(&isl->lock_irq);
	mutex_init(&isl->lock);
	mutex_init(&isl->bit_lock);

	isl->irqCount = 0;
	isl29030_enable_irq_wake(isl, client->irq);

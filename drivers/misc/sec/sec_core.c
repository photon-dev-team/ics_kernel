...
{
	int buffer_len=0;
	unsigned char sbk_data[TegraFuseSizeInBytes_SecureBootKey+1] = { 0 };
	unsigned char sbk_temp[TegraFuseSizeInBytes_SecureBootKey+1] = { 0 };
	...
	case SEC_OP_SBK_STATUS: /* SBK programmed status */
		*((unsigned int *)data) = 0;
		buffer_len = sizeof(unsigned int);
		if ((ReadByteFuseData(NvDdkFuseDataType_SecureBootKey,
				sbk_data)) == NvSuccess) {
			if (memcmp(sbk_data, sbk_temp,
				TegraFuseSizeInBytes_SecureBootKey) > 0)
				*((unsigned int *)data) = 1 ;
		}
		break;

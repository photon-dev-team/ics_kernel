...
/* Define the data which will be displayed by the debugfs entry for wakeup.
 */
# define WAKE_LOG_SZ 64  /* Must be a power of 2 for the circular
						    queue to work. */
struct wake_log_s
{
	unsigned int in;
...
static struct wake_log_s *wake_log_p = NULL;
#endif
...
#ifdef CONFIG_DEBUG_FS
static int wake_log_show(struct seq_file *s, void *unused)
{
	int i;

	seq_printf(s, "Time                 Status\n");
	if (wake_log_p) {
		while ((wake_log_p->out != wake_log_p->in) &&
			   (s->count+32 < s->size)){
			i = (wake_log_p->out++)&WAKE_LOG_SZ;
			seq_printf(s, "%10ld.%09ld: 0x%08x\n",
					   wake_log_p->log[i].time.tv_sec,
					   wake_log_p->log[i].time.tv_nsec,
					   wake_log_p->log[i].status);
		}
	}
	return 0;
}

static int wake_log_open(struct inode *inode, struct file *file)
{
	return single_open(file, wake_log_show, &inode->i_private);
}

static const struct file_operations wake_log_fops = {
	.open		= wake_log_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
...
{
	/* Setup the debug file system entry for the wakeup log. */
	wake_log_p = (struct wake_log_s *)kmalloc(sizeof(struct wake_log_s), GFP_KERNEL);
	if (wake_log_p != NULL)
	{
		wake_log_p->in = 0;
		wake_log_p->out = 0;
		(void)debugfs_create_file("tegra_wakeup", S_IRUGO,
								  NULL, NULL, &wake_log_fops);
	}
	return 0;
}
...
static void wake_log_status(void)
{
	unsigned int i;

	if (wake_log_p) {
		i = (wake_log_p->in++)&WAKE_LOG_SZ;
		getnstimeofday(&(wake_log_p->log[i].time));
		wake_log_p->log[i].status = readl(pmc+PMC_WAKE_STATUS);
	}
}
#else
# define wake_log_status()
#endif
...
static int tegra_suspend_prepare_late(void)
{
	...
	if (e != NvSuccess) {
		pr_err("%s: SendMsgInLP0 returning 0x%08x\n", __func__, e);
	}
